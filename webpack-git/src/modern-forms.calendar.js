import './moder-forms-input';

class mskinCalendar extends mskinInput {
    ...
    constructor( targetElement, startDate=null ) {
        ...
        /*this.htmlElement.container= targetElement;
		...
		this.events.eventFunctions["fnClickTextbox"] = function( mainScope ) {
			mainScope.showDialog( )
		};
		this.events.eventFunctions["fnClickMonthBack"] = function( ) {
		...
		this.events.eventFunctions["fnClickMonthForward"] = function( ) {
		...
		this.events.eventFunctions["fnClickYearBack"] = function( ) {
			var currDate = oldScope.displayDate;
			oldScope.setCalendarMonth( currDate.getFullYear() -1, currDate.getMonth() );
		};
		this.events.eventFunctions["fnClickYearForward"] = function( ) {
			var currDate = oldScope.displayDate;
			oldScope.setCalendarMonth( currDate.getFullYear() +1, currDate.getMonth() );
		};
		this.events.eventFunctions["fnClickDay"] = function( creatorScope, passedParam, clickedDay ) {
		...
		this.events.eventFunctions["fnClickTermin"] = function( el ) {

		};
		this.events.eventFunctions["fnClickCloseDatepicker"] = function( ) {

		};
		this.events.eventFunctions["fnChangeSelectedDay"] = function( creatorScope, passedParam, evParam ) {
		...
		this.events.eventFunctions["fnChangeDisplayedMonth"] = function( creatorScope, passedParam, evParam ) {
			...
			&&  newDate.getMonth() === nowDate.getMonth() ) {
				...
				todayDateArray[1] = ( (todayDateArray[1][0] === "0" ) ? todayDateArray[1][1] : todayDateArray[1] );
		...
		this.events.eventFunctions["onChangeVisibility"] = function( newState ) {

		};


		/* OnClick Events */
		this.events.eventHandle["onClickMonthBack"] = [];
		this.events.eventHandle["onClickMonthForward"] = [];
		this.events.eventHandle["onClickYearBack"] = [];
		this.events.eventHandle["onClickYearForward"] = [];
		this.events.eventHandle["onClickDay"] = [];
		this.events.eventHandle["onClickTermin"] = [];
		this.events.eventHandle["onClickCloseDatepicker"] = [];
		this.events.eventHandle["onClickTextbox"] = [ new CoEvent( this.events.eventFunctions.fnClickTextbox, this, { type: "onClickTextbox" } ) ];

		/* OnChange Events */
		this.events.eventHandle["onChangeDisplayedMonth"] = [];
		this.events.eventHandle["onChangeDate"] = [];
		this.events.eventHandle["onChangeVisibility"] = [];
...
/*        this.events = {
    ...
    defineAllDefaultFunction() {
        ...
        var fnChangeSelectedDay = this.events.eventFunctions["fnChangeSelectedDay"];
        var evChangeSelectedDay = new CoEvent( fnChangeSelectedDay, this );
        this.events.eventHandle.onChangeDate.push( evChangeSelectedDay );

        // this.events.eventFunctions["fnClickDay"]
        var fnClickDay = this.events.eventFunctions["fnClickDay"];
        var evClickDay = new CoEvent( fnClickDay, this );
        this.events.eventHandle.onClickDay.push( evClickDay );

        // this.events.eventFunctions["fnChangeDisplayedMonth"]
        var fnChangeDisplayedMonth = this.events.eventFunctions["fnChangeDisplayedMonth"];
        var evChangeDisplayedMonth = new CoEvent( fnChangeDisplayedMonth, this );
        this.events.eventHandle.onChangeDisplayedMonth.push( evChangeDisplayedMonth );
    }

    defineAllExternalDefaultFunction() { }

    triggerOnChangeDate( newDate ) {
        for( var iEvent = 0; iEvent < this.events.eventHandle.onChangeDate.length; iEvent++ ) {
            this.events.eventHandle.onChangeDate[iEvent].fire( {
    ...
    triggerOnClicketDay( evtClick ) {
        for( var iEvent = 0; iEvent < this.events.eventHandle.onClickDay.length; iEvent++ ) {
            this.events.eventHandle.onClickDay[iEvent].fire( evtClick );
        }
    }

    triggerOnChangeDisplayedMonth( newDate ) {
		var events = this.events.eventHandle.onChangeDisplayedMonth;
    ...
    bindMonthChangeListeners() {
        ...
        $( ".mskin-dp-month button[job=back]", this.htmlElement.container).click(
            // Übergebe Prototype als Event-Funktion
            this.events.eventFunctions["fnClickMonthBack"]
        );

        $( ".mskin-dp-month button[job=forward]", this.htmlElement.container).click(
            // Übergebe Prototype als Event-Funktion
            this.events.eventFunctions["fnClickMonthForward"]
        );
    }
    ...
    bindYearChangeListeners() {
        ...
        $( ".mskin-dp-year button[job=back]", this.htmlElement.container).click(
            // Übergebe Prototype als Event-Funktion
            this.events.eventFunctions["fnClickYearBack"]
        );

        $( ".mskin-dp-year button[job=forward]", this.htmlElement.container).click(
            // Übergebe Prototype als Event-Funktion
            this.events.eventFunctions["fnClickYearForward"]
        );
    }
    ...
    /* --- Private Methoden --- */
        ...
        draw_popup( date ) {
            var oldScope = this;
            $( this.htmlElement.container ).append(
    ...
    setCalendarMonth( year, month ) {
        ...
        $( ".mskin-dp-c-b-days .mskin-dp-c-b-d-container day", this.htmlElement.container).click( dayOnClickRedirectFn );
    }

    changeCalendarScope( daysHTML, directionNext=true ) {
        var oldScope = this;
        var dayContainerCollection = $( ".mskin-dp-c-b-days", this.htmlElement.container);
     ...
     */
    showGUI( ) {
        $( ".mskin-datepicker-container", this.htmlElement.container).addClass( "show" );
        this.fireCallback( this.events.eventHandle.onChangeVisibility, true );
    }
     ...
     * @param {function} newFn [optional] - Übergebe eine Function um diese zu den Events hinzufügesn zu lassen
     */
    onChangeVisibility( newFn=false ) {
        if( newFn ) {
            if( typeof newFN === "function" ) {
                this.events.eventHandle.onShowDatepicker.push( newFn );
            }
            else {
            ...
            var allOnChangeVisibilityEvents = this.events.eventHandle.onChangeVisibility;
    ...
    onChangeDate( newFn=false ) {
        if( newFn ) {
            if( typeof newFN === "function" ) {
                this.events.eventHandle.onChangeDate.push( newFn );
            }
            else {
                console.error( "Übergebenes Objekt zum hinzufügen der Eventsammlung ist keine Funktion! Als Parameter sind nur Objekte des Typs 'function' zugelassen! " );
            }
            ...
            var allOnChangeDateEvents = this.events.eventHandle.onChangeDate;
    ...
    onClickDay( newFn=false ) {
        if( newFn ) {
            if( typeof newFN === "function" ) {
                this.events.eventHandle.onClickDay.push( newFn );
            }
            else {
                console.error( "Übergebenes Objekt zum hinzufügen der Eventsammlung ist keine Funktion! Als Parameter sind nur Objekte des Typs 'function' zugelassen! " );
            }
            ...
            var allOnClickDayEvents = this.events.eventHandle.onClickDay;
    ...
    onClickTermin( newFn=false ) {
        if( newFn ) {
            if( typeof newFN === "function" ) {
                this.events.eventHandle.onClickTermin.push( newFn );
            }
            else {
                console.error( "Übergebenes Objekt zum hinzufügen der Eventsammlung ist keine Funktion! Als Parameter sind nur Objekte des Typs 'function' zugelassen! " );
            }
            ...
            var allOnClickTerminEvents = this.events.eventHandle.onClickTermin;
    ...
    date( setDate=false ) {
        if( setDate ) {
            ...
            this.fireCallback( this.events.eventHandle.onChangeDate );
        }
    ...
    mode( setMode="", msg=null ) {
		...
		if( ( setMode === "info" || setMode === "error" ) && msg !== null ) {
			if( $( this.htmlElement.container).find( ".mskin-object-header" ).length === 0 ) {
				...
				$( ".material-icons", this.htmlElement.container)
        ...
        if( setMode === "info" && msg !== null ) {
            ...
            $( this.htmlElement.container).addClass( "has-info" );
            $( this.htmlElement.container).removeClass( "has-error" );
            this.modeMsg = msg;

			$( ".mskin-textbox-input", this.htmlElement.container)
				.attr( "placeholder", "" );
            ...
            this.modeName = setMode;
            $( this.htmlElement.container).addClass( "has-error" );
            $( this.htmlElement.container).removeClass( "has-info" );
            this.modeMsg = msg;
			...
			$( this.htmlElement.container).addClass( "has-value" );
        }
        else if( setMode === "none" ) {
            this.modeName = setMode;
            this.modeMsg = null;

			var datePickerContainer = $( this.htmlElement.container);
			var txtBox = $( ".mskin-textbox-input", this.htmlElement.container);
			...
			if( txtBox.val( ) == "" ) {
				$( this.htmlElement.container).removeClass( "has-value" );
			}
        }
    ...
    showDialog() {
        if( !this.isShown ) {
            ...
            this.isShown = true;
            this.showGUI();
            this.fireCallback( this.events.eventHandle.onChangeVisibility );
        }
    }
	...
	triggerOnClickTextbox( oldScope, evtInfo, newValue ) {
		for( var iEvent = 0; iEvent < oldScope.events.eventHandle.onClickTextbox.length; iEvent++ ) {
			oldScope.events.eventHandle.onClickTextbox[iEvent].fire( newValue );
		}
	}
}
// var calendar = null;
...
//     calendar = new mskinCalendar( date_picker, new Date( ) );
// });
